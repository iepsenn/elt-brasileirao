...
class Scrapper():
    def __init__(self, url_per_year):
        self.url_per_year = url_per_year
        self.url = "https://fbref.com/"
        self.words_black_list = ['', '\xa0', '\n']
        self.columns_names = {
            "#": "shirt_number",
            "team": "team",
            "match_id": "match_id",
                ...
                for stats_infos in stats_divs:
                    if 'div' in str(stats_infos):
                        for stats_infos_divs in stats_infos:
                            if str(stats_infos_divs) not in self.words_black_list:
                                match_stats.append(stats_infos_divs)
                    else:
                        if str(stats_infos) not in self.words_black_list:
    ...
    def run(self):
        for url_base in self.url_per_year:
            match_url = url_base
            ...
            for idx, match_url in enumerate(url_matches_list):
                request = requests.get(self.url + match_url)
                page_content = BeautifulSoup(request.content, 'html.parser')

                match_stats, home_team, away_team = (
                    self.get_match_stats(page_content)
                )
                ...
                for stats_index in range(0, len(match_stats), 3):
                    ...
                    stats_dict[
                        stats_name.lower().replace(' ', '_') + '_home'
                    ] = [home_team_stats]
                    stats_dict[
                        stats_name.lower().replace(' ', '_') + '_away'
                    ] = [away_team_stats]
                ...
                match_infos = [
                    info for info in
                    page_content.findAll('div', {'class': 'scorebox_meta'})[0]
                    if info not in self.words_black_list
                ]
                ...
                if len(geo_infos) > 1:
                    if len(geo_infos) == 3:
                        stadium, state, UF = [
                            geo_info for geo_info in
                            match_infos[3].findAll('small')[1]
                        ][0].split(', ')
                    else:
                        stadium, state = [
                            geo_info for geo_info in
                    ...
                    stadium = state = UF = ''
                round = ([info for info in match_infos[1]][1]
                         .replace(' ', '')
                         .replace('(', '')
                         .replace(')', '')
                         .replace('Matchweek', '')
            ...
            match_team_stats.columns = [
                self.columns_names[col] if col in self.columns_names.keys()
            ...
            df_match_infos.to_csv(
                f'../data/{year}_match_infos.csv',
                index=False
            )
            match_team_stats.to_csv(
                f'../data/{year}_match_team_stats.csv',
                index=False
            )
            match_gk_stats.to_csv(
                f'../data/{year}_match_gk_stats.csv',
                index=False
            )
            all_matches.to_csv(
                f'../data/{year}_all_matches.csv',
                index=False
            )

            print(f'DONE for url: {url_base}')
            sleep(60 * 5)


if __name__ == "__main__":
    Scrapper(url_per_year=url_base_per_year).run()
